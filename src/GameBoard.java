...
public class GameBoard {
    ...
    public void changeActivePlayer() {
        ...
        if (!players.get(activePlayer).inGame)
            changeActivePlayer();
        else if(players.get(activePlayer).inPrison){
            server.sendServerMsg("Игрок " + players.get(activePlayer).name + " пропускает ход" );
            players.get(activePlayer).skipMove();
            server.sendPl(activePlayer);
            changeActivePlayer();
        }
        players.get(activePlayer).doubleNum = 0;
    }
    ...
    public void move(Messages.MoveMsg move) {
        ...
        if (curPl.move(dice1,dice2))
            server.sendServerMsg("Игрок " + curPl.name + " проходит поле Вперед и получает 200" );
        server.sendPl(activePlayer);
        ...
        if(cards[pos] instanceof PropertyCard && !((PropertyCard)cards[pos]).isFree && move.idPlayer != ((PropertyCard)cards[pos]).owner.id) {
            ...
            if (curPl.money >= tax) {
                server.sendServerMsg("Игрок " + curPl.name + " платит банку " + ((PayCard) cards[pos]).getTax() );
                curPl.pay(tax);
                server.sendPl(activePlayer);
            }
            else {
        ...
        else if (cards[pos] instanceof ChanceCard){
            ((ChanceCard) cards[pos]).setChance();
            server.sendServerMsg(((ChanceCard)cards[pos]).getDescr());
            if (((ChanceCard)cards[pos]).chance.type == 1){
                int tax = ((ChanceCard) cards[pos]).getTax();
                if (curPl.money >= tax) {
                    curPl.pay(tax);
                    server.sendPl(activePlayer);
                }
                ...
                move.Dice1 = 0;
                int moveTo = ((ChanceCard)cards[pos]).chance.moveTo;
                if(moveTo < curPl.position)
                    moveTo += 40;
                move.Dice2 = moveTo - curPl.position;
                move(move);
            }
        }
        else if (cards[pos] instanceof GoToPrisonCard){
            server.sendServerMsg("Игрок " + curPl.name + " отправляется в тюрьму");
            curPl.goToPrison();
            server.sendPl(activePlayer);
        }
    }
    ...
    public void initGameBoard(){
        ...
        cards[2] = new ChanceCard(2,"Казна",148,0);
        ...
        cards[7] = new ChanceCard(7,"Шанс",438,0);
        ...
        cards[17] = new ChanceCard(17,"Казна",610,438);
        ...
        cards[22] = new ChanceCard(22,"Шанс",494,610);
        ...
        cards[30] = new GoToPrisonCard(30,"Отправляйтесь в тюрьму",0,610);
        //
        ...
        cards[33] = new ChanceCard(33,"Казна",0,436);
        ...
        cards[36] = new ChanceCard(36,"Шанс",0,262);
